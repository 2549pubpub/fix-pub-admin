...
function backupData() {
    ...
    a.href = url;
    a.download = "fixpub-backup-" + date + ".json";
...
function restoreData(event) {
    ...
    reader.onload = function(e) {
        ...
        localStorage.setItem("repairs", e.target.result);
...
function generateIncomeChart() {
    ...
    repairs.forEach(repair => {
        ...
        if (repair.status === "เสร็จแล้ว") {

            const date = repair.date || "ไม่ระบุ";
...
function displayStock(){
    ...
    const valueText = document.getElementById("stock-value");
    ...
    stock.forEach(item => {
        ...
        const low = item.qty <= 3 ? "style='color:red;font-weight:bold'" : "";
        ...
        <tr>
            ...
            <td>${total}</td>
            <td>
                <button onclick="editStock(${item.id})">แก้ไข</button>
                <button onclick="deleteStock(${item.id})">ลบ</button>
            </td>
        </tr>
        `;
    ...
    valueText.innerText =
        "มูลค่าสต๊อกทั้งหมด: " + totalValue.toLocaleString() + " บาท";

}

function deleteStock(id){

    if(!confirm("ลบอะไหล่?")) return;
...
function editStock(id){
    ...
    const newQty = prompt("จำนวนใหม่:", item.qty);
